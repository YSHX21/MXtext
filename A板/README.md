...
本仓库同时包含两个独立可编译的 Keil 工程：

| 目录 | 节点 | 角色 | 说明 |
|---|---|---|---|
| 仓库根目录 | **A · 主控** | 调度 / 门控 / 称重 / 显示 | FreeRTOS 多任务，`MDK-ARM/MXtext.uvprojx` |
| `NodeB/` | **B · 驱动** | 曳引执行 / 位置闭环 | 裸机 + 定时器中断节拍，`NodeB/MDK-ARM/fastdianji.uvprojx` |

**技术栈**：`STM32F103` `FreeRTOS` `CAN2.0A` `ADC+DMA` `TIM-PWM` `TIM-Encoder` `I2C` `HX711` `状态机` `HAL`
...
| CAN 真实双节点联调（两板对拉） | 🚧 待实机验证 |
| 串级 PID（当前为位置闭环 + 分段速度给定） | 🚧 待升级 |
...
| TB6612FNG + 霍尔编码器减速电机 | 1 | 曳引执行（11PPR × 377:1 减速比） |
| 微动限位开关 | 2 | 🚧 上下限位，待接入 |
...
**波特率**：125 kbps（`PCLK1=36MHz`，`PSC=48`，`BS1=2TQ`，`BS2=3TQ`，`SJW=2TQ`，采样点 50%）

> ⚠️ 两个节点的位时序必须**逐参数一致**，任一参数不同都会导致采样时刻错位。
> A、B 两个工程当前使用完全相同的一套配置，改动时务必两边同改。

协议定义的唯一权威是 `NodeB/Core/Inc/can_proto.h`（B 板侧），
A 板侧原始定义在 `Core/Src/app.c` 中 —— 两者必须保持同步。
...
1. Keil MDK 打开 `MDK-ARM/MXtext.uvprojx`
2. 按需修改 `Core/Inc/app.h` 中的运行模式：

```c
#define MODE_PURE_LOCAL  0   /* 完全不用CAN，纯本地延时逻辑 */
#define MODE_LOCAL_SIM   1   /* 单板自测：CAN回环 + 本地曳引模拟 */
#define MODE_DUAL_NODE   2   /* 真双节点：A只管调度，升降由驱动板B执行【默认】 */
...
1. Keil MDK 打开 `NodeB/MDK-ARM/fastdianji.uvprojx`
2. 单板自测（不接 A 板）时，在 `NodeB/Core/Inc/can.h` 打开回环，
   并在 `NodeB/Core/Inc/traction.h` 打开台架自动跑楼：
...
**9. CAN1 走 PB8/PB9 必须显式打开重映射**
STM32F1 的 CAN1 默认落在 PA11/PA12。引脚配成 PB8/PB9 后若不调
`__HAL_AFIO_REMAP_CAN1_2()`，**CAN 控制器仍然挂在 PA11/PA12 上**，
一帧都收发不出去，而且**不会有任何报错** —— 极难查。
...
**必须实测标定**。（`encoder.c` 里提供了 `Encoder_CalStart/CalStop` 自检标定流程）
...
**13. 用板载 LED 做诊断前，必须先证明 LED 本身是好的**
"编码器不闪"和"LED 坏 / LED 引脚假设错了"现象完全一样，会导向错误结论。
修复：上电先让 LED **常亮 600 ms** —— 看到这段常亮，后续"不闪"才有诊断意义。

**14. 手转测编码器时必须先关驱动器**
TB6612 在 PWM=0 且 AIN1≠AIN2 时是"短路制动"，377:1 的减速箱轴硬得基本拧不动。
不先 `Motor_Disable()`（输出高阻）就手转，会得出"编码器坏了"的错误结论。
...
- [ ] CAN 真实双节点联调（两板对拉）
- [ ] 位置环 + 速度环**串级 PID**（当前是位置闭环 + 分段速度给定）
- [ ] 梯形加减速曲线 + 平层算法（替代当前的线性降速 + 到站窗口）
- [ ] 井道自学习（上电自动标定各楼层绝对脉冲坐标，存 Flash）
- [ ] 端站限位校正，消除累积误差
- [ ] 超载报警（HX711 阈值判断 + 蜂鸣器）
- [ ] 独立看门狗 + CAN 心跳超时保护
- [ ] 抽取 `can_proto.h` 为两工程共用协议头（当前 A 板侧定义在 `app.c` 内）
